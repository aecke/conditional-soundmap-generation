...
def forward_and_loss(args, params, model, left_batch, right_batch, extra_cond_batch):
    ...
    if "c_glow" in args.model:
        ...
        left_glow_outs, right_glow_outs = model(
            x_a=noise_added(left_batch, n_bins),
            x_b=noise_added(right_batch, n_bins),
            extra_cond=extra_cond_batch,
        )
        log_p_left, log_det_left = (
            left_glow_outs["log_p"].mean(),
            left_glow_outs["log_det"].mean(),
        )
        log_p_right, log_det_right = (
            right_glow_outs["log_p"].mean(),
            right_glow_outs["log_det"].mean(),
        )

        loss_left, _, _ = calc_loss(
            log_p_left, log_det_left, params["img_size"], n_bins
        )
        loss_right, _, _ = calc_loss(
            log_p_right, log_det_right, params["img_size"], n_bins
        )
...
def calc_loss(log_p, logdet, image_size, n_bins):
    ...
    loss = (
        -log(n_bins) * n_pixel
    )  # -c in Eq. 2 of the Glow paper, discretization level = 1/256 (for instance)
...
def compute_val_bpd(args, params, model, val_loader):
    ...
    print(
        f"====== In [train]: val_loss mean: {round(val_loss_mean, 3)} - loss_right_mean: {round(loss_right_mean, 3)}"
    )
...
def calc_val_loss(args, params, model, val_loader):
    print(
        f'In [calc_val_loss]: computing validation loss for data loader of len: {len(val_loader)} '
